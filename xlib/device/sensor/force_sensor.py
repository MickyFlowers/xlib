...
class Ft300sSensor:
    def __init__(self, port, timeout, zero_reset=True):
        client = ModbusClient(
            method="rtu", port=port, stopbits=1, bytesize=8, parity="N", baudrate=19200, timeout=timeout
        )
        client.connect()
    ...
    def get_force_torque_raw(self):
        ...
        force_torque = [
            int.from_bytes(raw_bytes[i * 2 : i * 2 + 2], byteorder="little", signed=True) / coef_list[i]
            for i in range(0, 6)
        ]
        return force_torque
    ...
    def get_force_torque(self):
        ...
        return np.array([ft - ft_zero for ft, ft_zero in zip(self.get_force_torque_raw(), self.zero_force_torque)])
